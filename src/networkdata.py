...
class NetworkData:
    ...
    def get_net_data(self):
        return {'lane':self.lane_data, 'edge':self.edge_data,
                'origin':self.find_origin_edges(), 'destination':self.find_destination_edges(),
                'node':self.node_data, 'inter':self.intersection_data}
    ...
    def get_edge_data(self, net):
        ...
        for edge in edges:
            ...
            edge_data[edge_ID]['coord'] = np.array([incnode_coord[0], incnode_coord[1],
                                                    outnode_coord[0], outnode_coord[1]]).reshape(2,2)
    ...
    def get_node_data(self, net):
        ...
        for node in nodes:
            ...
            node_data[node_id]['tlsindex'] = {conn.getTLLinkIndex():str(conn.getFromLane().getID())
                                               for conn in node.getConnections()}
            node_data[node_id]['tlsindexdir'] = {conn.getTLLinkIndex():str(conn.getDirection())
                                                  for conn in node.getConnections()}
        ...
        intersection_data = {str(node):node_data[node] for node in node_data
